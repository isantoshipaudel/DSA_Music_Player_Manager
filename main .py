...
class MusicApp:
    def __init__(self, root):
        ...
        # Main page (library + playlist)
        main_frame = tk.Frame(notebook, bg="#E8E8E8")
        notebook.add(main_frame, text="Main")

        library_frame = tk.Frame(main_frame, bg="#E8E8E8")
        library_frame.pack(side=tk.LEFT, padx=15, pady=15, fill=tk.Y)

        playlist_frame = tk.Frame(main_frame, bg="#E8E8E8")
        playlist_frame.pack(side=tk.RIGHT, padx=15, pady=15, fill=tk.BOTH, expand=True)

        # ===================== Library Section =====================
        tk.Label(library_frame, text="🎧 Music Library", bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 14, "bold")).pack(pady=5)
        self.library_listbox = tk.Listbox(library_frame, width=40, height=20, selectmode=tk.MULTIPLE, bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 11))
        self.library_listbox.pack(pady=5)

        tk.Button(library_frame, text="📂 Load Folder", command=self.load_folder, bg="#388E3C", fg="#E8E8E8", width=22, height=2).pack(pady=5)
        tk.Button(library_frame, text="➕ Add to Playlist", command=self.add_to_playlist, bg="#1976D2", fg="#E8E8E8", width=22, height=2).pack(pady=5)
        tk.Button(library_frame, text="🗑 Delete Song", command=self.delete_library_song, bg="#FBC02D", fg="black", width=22, height=2).pack(pady=5)

        # ===================== Playlist Section =====================
        tk.Label(playlist_frame, text="🎶 Playlists", bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 14, "bold")).pack(pady=5)
        self.playlist_listbox = tk.Listbox(playlist_frame, width=35, height=6, bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 11))
        self.playlist_listbox.pack(pady=5)
        self.playlist_listbox.bind("<<ListboxSelect>>", self.select_playlist)

        btn_frame_playlist = tk.Frame(playlist_frame, bg="#E8E8E8")
        btn_frame_playlist.pack(pady=5)
        tk.Button(btn_frame_playlist, text="🆕 New Playlist", command=self.new_playlist, bg="#388E3C", fg="#E8E8E8", width=15, height=2).grid(row=0, column=0, padx=5)
        tk.Button(btn_frame_playlist, text="🗑 Delete Playlist", command=self.delete_playlist, bg="#F44336", fg="#E8E8E8", width=15, height=2).grid(row=0, column=1, padx=5)

        tk.Label(playlist_frame, text="🎵 Songs in Playlist", bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 14, "bold")).pack(pady=5)
        self.playlist_songs_listbox = tk.Listbox(playlist_frame, width=50, height=15, bg="#E8E8E8", fg="#388E3C", font=("Helvetica", 11))
        self.playlist_songs_listbox.pack(pady=5)

        btn_frame_songs = tk.Frame(playlist_frame, bg="#E8E8E8")
        btn_frame_songs.pack(pady=5)
        tk.Button(btn_frame_songs, text="🗑 Delete Song", command=self.delete_playlist_song, bg="#F44336", fg="#E8E8E8", width=15, height=2).grid(row=0, column=0, padx=5)
        tk.Button(btn_frame_songs, text="🔀 Shuffle", command=self.shuffle_playlist, bg="#1976D2", fg="#E8E8E8", width=15, height=2).grid(row=0, column=1, padx=5)
        tk.Button(btn_frame_songs, text="⭐ Upvote", command=self.upvote_song, bg="#FF9800", fg="#E8E8E8", width=15, height=2).grid(row=0, column=2, padx=5)
        tk.Button(btn_frame_songs, text="⏩ Play Next", command=self.queue_play_next, bg="#9C27B0", fg="#E8E8E8", width=15, height=2).grid(row=0, column=3, padx=5)

        # ===================== Playback Controls =====================
        control_frame = tk.Frame(playlist_frame, bg="#E8E8E8")
        control_frame.pack(pady=10)
        tk.Button(control_frame, text="▶ Play", command=self.play_song, bg="#388E3C", fg="#E8E8E8", width=15, height=2).grid(row=0, column=0, padx=5, pady=5)
        tk.Button(control_frame, text="⏸ Pause/Resume", command=self.toggle_pause, bg="#1976D2", fg="#E8E8E8", width=15, height=2).grid(row=0, column=1, padx=5, pady=5)
        tk.Button(control_frame, text="⏮ Previous", command=self.previous_song, bg="#FBC02D", fg="black", width=15, height=2).grid(row=0, column=2, padx=5, pady=5)
        tk.Button(control_frame, text="⏭ Next", command=self.next_song, bg="#FF9800", fg="#E8E8E8", width=15, height=2).grid(row=0, column=3, padx=5, pady=5)

        self.slider_dragging = False
        # Slider frame
        self.slider_frame = tk.Frame(playlist_frame, bg="#E8E8E8")
        self.slider_frame.pack(pady=10)
        self.slider = ttk.Scale(self.slider_frame, from_=0, to=100, orient=HORIZONTAL, value=0, length=900)
        self.slider.pack(padx=10, pady=(0,5))
        self.slider.bind("<Button-1>", self.slider_press)
        self.slider.bind("<ButtonRelease-1>", self.slider_release)
        self.time_label = tk.Label(self.slider_frame, text="00:00 / 00:00", bg="#2B2B2B", fg="#E8E8E8", font=("Helvetica", 10))
        self.time_label.pack(pady=(0,5))

        # ===================== Play History Section (new tab) =====================
        history_tab = tk.Frame(notebook, bg="#2B2B2B")
        notebook.add(history_tab, text="Playback History")
        tk.Label(history_tab, text="🕒 Play History", bg="#2B2B2B", fg="#E8E8E8", font=("Helvetica", 14, "bold")).pack(pady=5)
        self.history_listbox = tk.Listbox(history_tab, width=100, height=25, bg="#1E1E1E", fg="#E8E8E8", font=("Helvetica", 11))
